   ...
   view=TRUE, clean=TRUE, runTestFileInOwnProcess=TRUE, testLocal=FALSE )
{
    ...
    {
      stop(outdir, " is not a writable directory. Please ensure that the 'outdir' argument of IOQR() defines a path to a writable directory.")
    }
    ...
    IOQR:::copyWorkspace(".GlobalEnv", target.name=tempdb)

    # define the RUnit test suite and run tests
    testDirRoot <- system.file("unitTests", package="IOQR")

    # TODO: Consider if this can be done better:
    #testDirs <- unlist(lapply(file.path(testDirRoot,c("R")), function(x, os) file.path(x, c("common",os)), os=.Platform$OS.type))
    # unlist(lapply(file.path(testDirRoot,c("R")), function(x, os) file.path(x, c("common",os)), os=.Platform$OS.type))
    testDirs <- c( file.path(testDirRoot,c("R"),"common") )
    ...
    testResult <- runRTestSuite(testSuite, runTestFileInOwnProcess=runTestFileInOwnProcess)
    ...
    if (printJUnit)
    {
        ...
        printJUnitProtocol(testResult, fileName=textfile, showDetails=TRUE)
    }
    ...
    IOQR:::restoreWorkspace(target.name=tempdb)
...
"saveRUnitSession" <- function(opt=options(), packages=NULL, datasets=NULL, envir=.GlobalEnv)
{
    ...
    ##
    ## state       : list. saved session information as output by IOQR:::saveRUnitSessionRUnitSession().
    ##
...
"restoreRUnitSession" <- function(state=IOQR:::saveRUnitSessionRUnitSession())
{
...
"restoreWorkspace" <- function(target.name="tempdb", append=TRUE, detach.copy=TRUE)
{
    ...
    IOQR:::copyWorkspace(source.name=x$target.name, target.name=x$source.name,
  ...
  quiet=TRUE)
{
   ...
   {
      IOQR:::testPackageLoadability(pkg = pkg, quiet = quiet)
   }
...
runRTestSuite <- function(testSuites, useOwnErrorHandler=TRUE, runTestFileInOwnProcess=FALSE) {
  ##@bdescr
  ...
  for (i in seq_along(testSuites)) {
	...
	if (runTestFileInOwnProcess) {
        ...
        storefile <- gsub("\\\\", "/", tempfile(pattern="IOQR"))
        save(.testLogger, testFile, testSuite, file=storefile)
        testscript <- gsub("\\\\", "/", tempfile(pattern="IOQRTest"))
        cat("library(IOQR)\n",
