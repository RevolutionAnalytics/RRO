...
session <- IOQR:::saveRUnitSession(packages=c("cluster"), datasets=c("votes.repub","iris"))

"clusplot.out.stress" <- function()
{
    library(cluster)
...
"test.clusplot.out.stress" <- function()
{
    res <- try(clusplot.out.stress())
    checkTrue(!is(res, "try-error"), msg="clusplot.out stress test failed")
}
