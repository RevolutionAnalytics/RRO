...
session <- IOQR:::saveRUnitSession(packages="cluster", datasets=c("flower","agriculture","animals"))

"daisy.ex.stress" <- function()
{
...
"test.daisy.ex.stress" <- function()
{
    res <- try(daisy.ex.stress())
    checkTrue(!is(res, "try-error"), msg="daisy.ex stress test failed")
}
