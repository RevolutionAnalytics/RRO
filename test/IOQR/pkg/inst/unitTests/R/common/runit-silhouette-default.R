...
session <- IOQR:::saveRUnitSession(packages=c("cluster"), datasets=c("iris"))

"silhouette.default.stress" <- function()
{
...
"test.silhouette.default.stress" <- function()
{
    res <- try(silhouette.default.stress())
    checkTrue(!is(res, "try-error"), msg="silhouette.default stress test failed")
}
