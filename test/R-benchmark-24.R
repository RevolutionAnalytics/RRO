# R Benchmark 2.4 (06/2007) [Simon Urbanek]
...
for (i in 1:runs) {
  a <- new("dgeMatrix", x = Rnorm(600*600), Dim = as.integer(c(600,600)))
  b <- as.double(1:600)
  invisible(gc())
  timing <- system.time({
...
cumulate <- 0; b <- 0
for (i in 1:runs) {
  a <- crossprod(new("dgeMatrix", x = Rnorm(900*900),
                       Dim = as.integer(c(900, 900))))
  invisible(gc())
...
cumulate <- 0; b <- 0
for (i in 1:runs) {
  a <- new("dgeMatrix", x = Rnorm(400*400), Dim = as.integer(c(400, 400)))
  invisible(gc())
